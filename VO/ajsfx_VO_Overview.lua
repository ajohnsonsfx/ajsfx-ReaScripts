...
function Trim.sweep_residue_names()
  ...
  -- A role track that is somebody's parent (Selects nested under Alts is not a
  -- shape this tool builds, but a hand-made session can be anything) must not
  -- be swept: a delivered take is supposed to be named.
  for i = #regions, 1, -1 do
    local _, tn = r.GetSetMediaTrackInfo_String(regions[i].track, "P_NAME", "", false)
    if vo.MarkFromTrack(tn, cfg) ~= nil then table.remove(regions, i) end
  end
...
local REMOTE_HELP =
  ...
  "straighten_names | clear_residue | " ..
  "sort script|record | " ..
