...
function Trim.settle(opts)
  ...
  for pass = 1, max_passes do
    ...
    local res = Trim.cut_from_markers({ quiet = true }) or {}
  ...
  -- Items this run SPLIT no longer exist, so a stale pointer is not selectable;
  -- REAPER tolerates that, and the survivors come back selected.
  r.Main_OnCommand(40289, 0)            -- Unselect all items
  for _, it in ipairs(was_selected) do
    pcall(r.SetMediaItemSelected, it, true)
  end
  r.UpdateArrange()
  ...
  return { applied = applied_total, log = log }
end
...
local REMOTE_HELP =
  ...
  "settle | unpick | pick first|last | make_select <takename> | place | tighten | trim_to_markers"
