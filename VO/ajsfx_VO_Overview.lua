...
function RoleNames.Plan()
  ...
  return vo.PlanRoleNames(rows, known, cfg)
end
...
function RoleNames.Apply(auto)
  ...
  state.name_baseline = nil
  core.Transaction("VO Overview: names follow tracks", function()
  ...
  Rebuild()
  state.message, state.message_kind = string.format(
    "Renamed %d take(s) to match their tracks.%s", #plan.renames, conflicts),
    (conflicts ~= "") and "error" or "ok"
end
