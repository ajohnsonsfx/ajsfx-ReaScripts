...
local function Rebuild()
  ...
  -- The Qwen engine's decode context: the loaded lines, rewritten on every
  -- rebuild so a script edit reaches the next transcription (the transcribe
  -- cache keys on this file's content, so a change re-decodes). A Rebuild
  -- that lost its lines TRANSIENTLY -- a script CSV locked mid-autosave, a
  -- network path blinking -- must not delete the file: no context means the
  -- next decode silently goes blind under its own cache key, the exact
  -- regression the feature exists to prevent. Only a project with no
  -- scripts configured at all removes it, which is also what keeps one
  -- project's script from biasing another's decode. Review find, 2026-08-16.
  if #(state.lines or {}) > 0 or #(state.scripts or {}) == 0 then
    vo.WriteQwenContext(cfg, state.lines)
  end
  ...
  -- (The Suspects scan used to be dropped here, so that a held result could
  -- not outlive the rebuild that changed what it describes. It is CARRIED
  -- now instead -- see the vo.RescanChanged call below, which re-judges only
  -- the rows whose inputs actually moved. It has to run down there rather
  -- than here: the fields it fingerprints are filled in by the passes
  ...
  -- ONCE SCANNED, STAYS SCANNED (AJ). The free hunt survives the rebuild:
  -- rows whose inputs are unchanged keep their verdict, and only the rows
  -- that actually moved are re-judged. nil stays nil -- a scan nobody ran
  -- is never invented here.
  --
  -- The WORDS and the SCRIPT are what every verdict was earned against, so
  -- when either is replaced (a re-transcribe, a script reload -- both
  -- memoised, so their identity is stable through ordinary edits) every
  -- verdict is re-earned. Rare, and cheap beside whatever caused it.
  if state.suspects ~= nil then
    local stale = state.suspect_tx ~= state.transcripts
                  or state.suspect_lines ~= state.lines
    state.suspects, state.suspect_map = vo.RescanChanged(
      stale and {} or (state.suspect_map or {}),
      stale and {} or state.suspects,
      state.overview, state.transcripts or {}, state.lines or {},
      cfg, vo.VERIFY_THRESH)
    state.suspect_tx, state.suspect_lines = state.transcripts, state.lines
  end
  ...
  local pq_clean, pq_single, pq_by_key = {}, {}, {}
  ...
  for _, d in ipairs(vo.ParityDiff(pq_takes,
    ...
    -- deciding fact -- the words -- to be dug up by hand.
    local tk = pq_by_key[d.key]
    if tk and tk.marker and tk.path and pq_words[tk.path] then
      local under = vo.WordsInRange(pq_words[tk.path],
                                    tk.marker.start, tk.marker.stop)
      local said = {}
      for i = 1, math.min(#under, 8) do said[i] = under[i].text or "?" end
      d.detail = string.format("%s -- words: %s", d.detail or "out of sync",
        (#said > 0) and ('"' .. table.concat(said, " ") .. '"')
                     or "(none under the marker)")
    end
...
-- NAMES FOLLOW THE TRACKS: the drag is the decision, and the name catches
-- up. vo.PlanRoleNames owns the rules -- the BASE of a name still says which
-- line the take is (that never follows a drag), the track decides plain-
-- versus-alt, and anything ambiguous is reported rather than guessed. This
-- half reads the live project, applies the plan in one undo step, and never
-- touches an edge, a position or a trim: it renames, full stop.
local RoleNames = {}

function RoleNames.Plan(opts)
  local cfg = vo.LoadConfig()
  ...
  for i = 0, r.CountMediaItems(0) - 1 do
    ...
    if take then
      ...
      if nm and nm ~= "" then
        ...
        local tn = track_of[tr]
        if tn == nil then
          local _, got = r.GetSetMediaTrackInfo_String(tr, "P_NAME", "", false)
          tn = got or ""
          track_of[tr] = tn
        end
...
-- are never quiet -- a drag whose rename was refused has to say why.
-- `opts.renumber` is the STRAIGHTEN pass (manual only, never the watcher):
-- every line's alts renumbered compactly from _alt1 in timeline order.
function RoleNames.Apply(auto, opts)
  local plan = RoleNames.Plan(opts)
  local conflicts = ""
  if #plan.conflicts > 0 then
    local parts = {}
    for _, c in ipairs(plan.conflicts) do parts[#parts + 1] = c.detail end
    conflicts = " Left alone: " .. table.concat(parts, "; ") .. "."
  end
  if #plan.renames == 0 then
    if not auto or conflicts ~= "" then
      state.message, state.message_kind =
        (conflicts ~= "") and ("No names to fix." .. conflicts)
                          or  "Names and tracks already agree.",
        (conflicts ~= "") and "error" or "ok"
    end
    return
  end
  state.name_baseline = nil
  core.Transaction((opts and opts.renumber)
      and "VO Overview: straighten names"
      or  "VO Overview: names follow tracks", function()
    for _, rn in ipairs(plan.renames) do
      if r.ValidatePtr2(0, rn.row.take, "MediaItem_Take*") then
        r.GetSetMediaItemTakeInfo_String(rn.row.take, "P_NAME", rn.name, true)
      end
    end
    r.UpdateArrange()
  end)
  Rebuild()
  state.message, state.message_kind = string.format(
    (opts and opts.renumber)
      and "Straightened %d take name(s): selects plain, alts renumbered from 1.%s"
      or  "Renamed %d take(s) to match their tracks.%s",
    #plan.renames, conflicts),
    (conflicts ~= "") and "error" or "ok"
end
    ...
    for _, info in ipairs(state.items or {}) do
      ...
      if item and not info.skip and info.path and picked[item] then
        ...
        for _, mk in ipairs(list or {}) do
          ...
          -- Only OUR markers, and only ones the clip can PLAY: a marker
          -- outside the coverage window is split residue this verb neither
          -- judges nor lets claim the item's name -- the transcript macro
          -- drops those outright; here it is preserved untouched so the
          -- rename verb never deletes anything.
          local m0, m1 = mk.pos or 0, (mk.pos or 0) + (mk.length or 0)
          local in_cov = cov and vo.MarkerInItem(
            { start = m0, stop = m1 }, cov)
          if id and not vo.IsNoteMarker(mk.name or "") and not in_cov then
            out[#out + 1] = { start = m0, stop = m1, asset = asset, id = id }
          elseif id and not vo.IsNoteMarker(mk.name or "") then
            looked = looked + 1
            local now = asset
            -- WordsInRange, NOT WordsWithin: stored words are judged by their
            -- DTW ANCHOR, the same rule the sheet displays them by. The
            -- midpoint rule read a displaced whisper window as the take's
            -- words and judged a marker against text the audio does not play
            -- -- "agrees" about a marker the sheet itself showed refuted.
            -- WordsWithin stays correct for its own job, clipping FRESH
            -- decode output, where no anchors exist.
            local words = vo.WordsInRange(words_by_path[info.path], m0, m1)
            if #words == 0 then
...
function Trim.fix_from_transcript(opts)
  ...
  local pruned, hidden = 0, 0
  core.Transaction("VO Overview: fix from transcript", function()
    -- 0. HIDDEN markers: tool markers outside the clip's source coverage --
    -- split residue parked before or after the audio the clip can play.
    -- Invisible in the take list (which shows in-coverage markers only) and
    -- able to survive the mirror pass when no neighbouring clip claims
    -- them, they still count as "several markers" to every shape test and
    -- still sit first in chunk order, which is how one of them gets to name
    -- the item. The clip only owns what it can play: dropped outright,
    -- before anything judges anything.
    for _, info in ipairs(state.items or {}) do
      local item = info.item
      if item and not info.skip and picked[item] then
        local cov = vo.SourceCoverageRanges({ info })[1]
        -- NOT `cov and r.GetItemStateChunk(...)`: an `and` expression is
        -- adjusted to ONE value (markers_in learned this the hard way), so
        -- the chunk -- the second return -- would silently become nil.
        local okc, chunk = false, nil
        if cov then okc, chunk = r.GetItemStateChunk(item, "", false) end
        if okc and chunk then
          local keep, drop = {}, 0
          for _, m in ipairs(vo.ParseTKMChunk(chunk)) do
            local asset, id = vo.ParseMarkerName(m.name or "")
            if id then
              local e = { start = m.pos or 0,
                          stop = (m.pos or 0) + (m.length or 0),
                          asset = asset, id = id }
              if vo.MarkerInItem(e, cov) then
                keep[#keep + 1] = e
              else
                drop = drop + 1
              end
    ...
    -- The dupe pass below reads state.take_markers; the drops above have
    -- just made it stale for every item they touched.
    if hidden > 0 then
      state.take_markers = vo.CollectTakeMarkers(state.items)
    end
  ...
  if hidden > 0 then
    parts[#parts + 1] = string.format(
      "Dropped %d hidden marker(s) outside the audio their clip can play.",
      hidden)
  end
  for _, s in ipairs(Trim.dupe_report(plan, removed)) do parts[#parts + 1] = s end
  if dropped > 0 then
...
function Trim.sync_dispatch(attributed)
  ...
  if n.sheet > 0 then
    ...
    if adopted > 0 then
      ...
      -- alt used to swap the marks and leave both names describing the
      -- old roles. Generated names are re-roled; a name the user typed
      -- is kept, as everywhere.
      Trim.fix_names_from_sheet({ picked = by.sheet, quiet = true })
    end
  end
  if n.item > 0 then
...
local function DrawPullPanel()
  ...
  if im.IsItemHovered(ctx) then
    im.SetTooltip(ctx,
      ...
      "It only MOVES. The renaming half is the checkbox below.")
  end

  -- The mirror of the checkbox above: there a tick moves the item, here a
  -- drag renames the take.
  local nhit, nwant = im.Checkbox(ctx, "Names follow the tracks as I drag items",
                                  cfg.names_follow_tracks ~= false)
  if nhit then cfg.names_follow_tracks = nwant; vo.SaveConfig(cfg) end
  if im.IsItemHovered(ctx) then
    im.SetTooltip(ctx,
      "Drop a take on Selects and it takes the line's plain delivered\n" ..
      "name; drop it on Alts or Outs and it takes a numbered alt name.\n" ..
      "The BASE name -- which line it is -- never changes on a drag, and\n" ..
      "edges, positions and trims are never touched. Two takes left on\n" ..
      "Selects for one line is reported, not guessed.\n\n" ..
      "It only RENAMES. Moving items is the checkbox above, or Pull.")
  end
  im.SameLine(ctx)
  if im.Button(ctx, "Fix names now") then
    pending_action = function() RoleNames.Apply(false) end
  end
  if im.IsItemHovered(ctx) then
    im.SetTooltip(ctx,
      "One pass over the whole session, for drags made while the window\n" ..
      "was closed or the checkbox was off: every take on Selects, Alts or\n" ..
      "Outs whose name disagrees with its track is renamed to agree.\n" ..
      "Existing alt numbers are kept. One undo step.")
  end
  im.SameLine(ctx)
  if im.Button(ctx, "Straighten names") then
    pending_action = function() RoleNames.Apply(false, { renumber = true }) end
  end
  if im.IsItemHovered(ctx) then
    im.SetTooltip(ctx,
      "The stronger pass: every select gets its line's plain name, and\n" ..
      "every line's alts are RENUMBERED compactly from _alt1 in timeline\n" ..
      "order (Outs continue after the alts) -- gaps left by swaps and\n" ..
      "demotions close up. Alt numbers CHANGE meaning here: _alt2 may be\n" ..
      "a different take afterwards, so run it before delivering, not\n" ..
      "after anyone is referring to alt numbers. One undo step.")
  end
  im.Spacing(ctx)
...
function Verify.Judge(entry, fresh)
  ...
  -- Anchor rule for the STORED side: the staleness question is "does the
  -- audio still match what the sheet says this take is", and the sheet
  -- reads by anchor. The midpoint rule handed CompareWords a displaced
  -- window's words and called the sidecar stale about a take whose
  -- displayed transcript matched the audio exactly. (`fresh` stays
  -- midpoint-clipped above -- decode output has no anchors.)
  local stored = vo.WordsInRange(stored_all, entry.span.from, entry.span.to)
...
function Verify.QuickCheck(rows, notes)
  ...
  for _, e in ipairs(vo.PlanVerify(unjudged)) do
    -- Anchor rule (WordsInRange), the same one the sheet displays by: the
    -- quick check judges STORED words, and the midpoint rule handed it a
    -- displaced whisper window's text -- a verdict about words the sheet
    -- never showed. WordsWithin remains the clip rule for FRESH decodes.
    local words = vo.WordsInRange(by_path[e.source_path], e.span.from, e.span.to)
    if #words == 0 then
...
-- THE FREE HUNT, in one place. Three buttons offer it (the strip, the
-- Session card, the rescan row) and all three come here, so a scan can
-- never be started in a way that forgets to seed what carries it across
-- the next rebuild. Full scan every time: an empty prev map means every
-- row is re-judged.
function Inbox.RunScan()
  local cfg = vo.LoadConfig()
  state.suspects, state.suspect_map = vo.RescanChanged(
    {}, {}, state.overview or {}, state.transcripts or {},
    state.lines or {}, cfg, vo.VERIFY_THRESH)
  state.suspect_tx, state.suspect_lines = state.transcripts, state.lines
  state.message, state.message_kind = string.format(
    "Scanned %d row(s): %d worth a listen.",
    #(state.overview or {}), #(state.suspects or {})), "ok"
end
...
function Inbox.MaybeAssemble()
  ...
  for _, d in ipairs(rec.disagree or {}) do
    -- The stamp blesses only what its fingerprint witnessed: source span,
    -- edges, name, marker, words (vo.VettedFingerprint). TRACK placement is
    -- not in it -- an OK'd take dragged to the wrong track afterwards still
    -- carries a valid stamp about the old sight -- so marks-vs-track
    -- findings (every PlanReconcile detail names the track) are never
    -- silenced by it. Review find, 2026-08-16.
    local placement = d.detail and d.detail:find("track", 1, true)
    if placement
       or not (d.row and d.row.item and blessed[d.row.item]) then
  ...
  -- lookup loses track of hand-trimmed and comped items even though the
  -- NAME already assigns them -- so it flooded the list with hundreds of
  -- rows that were nobody's job. The Todo list is the reconciliation
...
-- A rail row is narrow; the full text lives in the tooltip. The cut
-- lands on a CODEPOINT boundary: a byte cut can split a multi-byte
-- character (the separator dot is one) and hand ImGui invalid UTF-8.
function Inbox.Clip(s, cap)
  ...
  return s:sub(1, cut) .. "\226\128\166"
end

-- A row's identity: the delivered name plus its take letter -- the way
-- AJ names a take out loud ("OldBookManSay B"), never a time range.
function Inbox.RowId(row)
  if not row then return "?" end
  local id = row.deliver or row.asset or "(unnamed)"
  if (row.take_index or 0) > 0 then
    id = id .. " " .. vo.TakeLetter(row.take_index)
  end
  return id
end
...
local function RunRemoteCommand(command)
  ...
  elseif verb == "todo" then
    ...
    -- "todo count" answers in one line: the whole list is a few kilobytes
    -- and a caller that only wants the number should not have to carry it.
    if rest == "count" then
      return string.format("todo %d (%d line(s), %d session)",
        counts.total or 0, counts.lines or 0, counts.session or 0)
    end
    local out = {}
    local todo = state.todo or {}
    -- "todo <needle>" is ONE line in full, evidence tips and all -- what
    -- the whole-list answer has to leave out to fit.
    if rest ~= "" then
      local needle = rest:lower()
      for _, e in ipairs(todo.lines or {}) do
        if tostring(e.label):lower():find(needle, 1, true) then
          out[#out + 1] = string.format("%s: %s", e.label, e.stage_label)
      ...
      return (#out > 0) and table.concat(out, "\n")
             or ("no line with work matches " .. rest)
    end
    for i, e in ipairs(todo.lines or {}) do
      ...
      -- Label only: the evidence tips are the long part, and 58 lines of
      -- them do not fit an answer. "todo <needle>" is the way to read one
      -- line's evidence in full.
      for _, f in ipairs(e.errors) do
        out[#out + 1] = string.format("      [%s] %s", f.kind, (Inbox.Label(f)))
      end
    end
  ...
  elseif verb == "suspects" then
    -- The free hunt, headless -- through the SAME entry the buttons use, so
    -- the seam cannot leave the scan in a state a press never produces.
    -- Timed: what it costs decides where it is allowed to run.
            ...
            if line and line.text and line.text ~= "" then
              ...
              if path and path ~= "" then
                ...
                if words then
                  ...
                  -- Anchor rule, like every stored-word judge: the overlap
                  -- rule graded takes against a displaced window's words.
                  local heard = {}
                  for _, w in ipairs(vo.WordsInRange(words, a, b)) do
                    heard[#heard + 1] = w.text
                  end
...
-- SEEDED FROM WHAT IS ALREADY THERE, not from zero. ExtState outlives the
-- script, so a fresh instance starting at 0 rewrites the very serial the
-- caller recorded before posting -- the answer lands, the serial does not
-- move, and every command after a restart reads as a hang no matter what
-- it did. Cost me an afternoon of blaming the wrong verb.
local remote_serial = tonumber(r.GetExtState(REMOTE_SECTION, "serial")) or 0
...
local function PollRemote()
  ...
  local text = ok and tostring(result) or ("ERROR: " .. tostring(result))
  -- A backstop, not a fix for anything seen: 4.3k answers round-trip fine,
  -- so the ceiling is somewhere above that and unmeasured. It exists so a
  -- verb that one day returns a whole transcript cannot take the seam down
  -- with it. Truncation is announced, never quiet -- a silently short
  -- answer read as fact is worse than no answer.
  local CAP = 16000
  if #text > CAP then
    text = text:sub(1, CAP) ..
      string.format("\n...truncated at %d of %d bytes", CAP, #text)
  end
  remote_serial = remote_serial + 1
  r.SetExtState(REMOTE_SECTION, "result", text, false)
  ...
  if visible then
    ...
    do
      ...
      if now ~= state.item_snapshot_at
        ...
        -- Any settled edit is a chance a take crossed a role track; the
        -- flag costs nothing and the sweep it buys runs once per batch,
        -- planning nothing when no name disagrees.
        state.role_names_dirty = true
        state.edit_settle = 0
      else
      ...
      if (state.edit_settle or 0) >= 15 and not pending_action then
        ...
        if redo and tostring(redo):find("^VO Overview") then
          ...
          -- And the rename sweep, or undoing "names follow tracks" would
          -- read as a fresh drag and redo the very rename just undone.
          state.role_names_dirty = nil
        end
        ...
        -- Names follow the tracks, after the parity dispatch: if the sync
        -- took this frame's action slot the flag simply survives to the
        -- next settle, so a drag is never half-handled. The sweep's own
        -- renames cannot re-fire it: the pending_action re-baseline below
        -- folds them into the snapshot like any other verb's work.
        --
        -- "In sync" is the MASTER switch over everything that mutates the
        -- project by itself -- its tooltip promises "Off: nothing runs by
        -- itself" -- so the sweep honours it like every other automatic
        -- pass (review find, 2026-08-16: it originally checked only its
        -- own Pull-panel toggle, and renamed drags the user had turned
        -- the watcher off to make in peace). "Fix names now" remains the
        -- manual path while sync is off.
        if state.role_names_dirty and not pending_action then
          state.role_names_dirty = nil
          if state.session_sync
             and vo.LoadConfig().names_follow_tracks ~= false then
            pending_action = function() RoleNames.Apply(true) end
          end
        end
      end
    end
