  ...
  if not anything then
    if opts.quiet then
      return { wrote = 0, named = 0, many = counts.many, none = counts.none,
               unusable = unusable, updated = 0, unchanged = 0, items = #items }
    end
    state.message, state.message_kind = string.format(
