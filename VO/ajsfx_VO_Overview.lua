...
function Inbox.Label(f)
  ...
  elseif k == "out_of_sync" and p.divergence then
    ...
    -- The queued item wears its own name; ask it rather than guess.
    local take = Trim.item_alive(p.item) and r.GetActiveTake(p.item)
    if take then
      local _, nm = r.GetSetMediaItemTakeInfo_String(take, "P_NAME", "", false)
