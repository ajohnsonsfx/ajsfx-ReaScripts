...
**Status:** Implemented 2026-08-14 · **Date:** 2026-08-14
...
### 5.1 What was actually built, and where it differs

- **`Trim.recut` on the existing `Trim` table, not a new top-level local.** The
  table is already the file's answer to the 200-local ceiling
  (`Trim.cut_from_markers`, `Trim.update`, `Trim.extras`), so the applier costs no
  local budget at all. Helpers `Trim.recut_items`, `Trim.recut_spans` and
  `Trim.recut_neighbours` sit beside it for the same reason.
- **`Trim.recut` is defined AFTER `MatchTakes`, and must be.** `MatchTakes` is a file
  local; a `Trim.recut` defined earlier compiles its call as a global lookup and finds
  `nil` at run time. `luac -p` cannot catch that — the check that does is
  `luac -l -l` and reading the `_ENV "…"` list for anything that is not stdlib or
  `reaper`.
- **Stripping the clump's markers is TWO calls, not one.** `vo.WriteTakeMarkers(item, {})`
  replaces only the lines carrying a ` ~id` and deliberately preserves everything
  else — and a note marker carries no id, so the stale `! PARTIAL:` complaint
  survives it untouched. `vo.WriteNoteMarker(item, pos, stamp, nil)` is what clears
  the notes; passing no text drops them all and writes none.
- **Spans come from `state.matches[].spans` tagged with `m.path`**, rebuilt into fresh
  tables by `Trim.recut_spans`. `CutCandidates` mutates the memoised span tables it
  walks, and a planner must not depend on whether a cut happened to run first.
- **The clump report reaches both of `MatchTakes`'s report paths** — `bits` for the
  sheet-only branch and `parts` for the button. Appending to only the first would
  have made the count invisible on the press that matters.
